# Track 1: DeepFense Framework

**SET OF EXPERIMENTS ⇒ TRACK1** 
We will start solely on **ASV19**.

## Overview

This document outlines the overall plan for Track 1. We are currently executing "Batch 1" (ASV19 focus with specific models), with further datasets and models planned as next steps.

## Experiment Table (Batch 1 - Current Focus)
...
**Frontends to be evaluated:**
*   **MERT-L**
*   **EAT-L**

**Backends to be evaluated:**
*   **Linear** (in addition to/instead of MLP)
*   **BiCrossMamba**

### Expanded Dataset Scope

Future experiments will expand beyond ASV19 to include:
...
*   CodecFake
*   Wavefake
*   Evaluated on: ASV21, DF21, ITW, MLAAD, FoR, [ReplayDF]
...
**Music:**
*   FakeMusicCaps [To be downloaded]

**Partial:**
*   HAD
*   PartialSpoof
*   EditSpoof [To be downloaded]

### Standard Pipeline
*   **Training**: 4s train and eval. Tuning the model. + CrossEntropy ⇒ reported metrics
*   **Augmentation**: RIR + Rawboost 3 (Already in use, but potentially tuning parameters)

## Configuration Notes

*   **Parquets**: Located in `/netscratch/yelkheir/DeepFense/DeepFense/deepfense/config/parquets`
    *   *Note*: Root paths need adaptation (e.g. in `generate_asv19.py`).
*   **Augmentations (RIR)**: Located in `/netscratch/yelkheir/DeepFense/DeepFense/deepfense/config/augmentations`
    *   *Note*: Root paths need adaptation.
